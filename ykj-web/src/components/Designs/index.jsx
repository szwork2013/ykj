...
const { confirm } = Modal;
const List = ({ form, designs, orderServiceAttachment, orderService, dispatch, ...rest }) => {
  const { getFieldProps } = form;
  const currentOrder = designs.currentOrder;
  ...
  return (
    <Container
      ...
      <Row>
        ...
        <Col span={12} style={{ textAlign: 'right' }}>
          <Button type="primary" size="large">
            <Link to={`/order/orders/${rest.params.id}/designs/add`}>创建测量安排</Link>
          </Button>
        </Col>
      </Row>
      <BoxTable
      ...
      <Modal
        visible={designs.attachmentModalShow}
        dispatch={dispatch}
        onCancel={() => {
          dispatch({
            type: 'designs/merge',
