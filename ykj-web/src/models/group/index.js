...
import { search, create, update, view, remove } from '../../services/group';
...
export default {
        ...
        listSubscriptions({ dispatch, history }) {
         ...
         editSubscriptions({ dispatch, history }) {
            ...
            history.listen((location, state) => {
                if (pathToRegexp('/customers/grouporder/edit/:id').test(location.pathname)) {
                const match = pathToRegexp('/customers/grouporder/edit/:id').exec(location.pathname);
                const id = match[1];
                console.info(id);
                    dispatch({
                        type: 'view',
    ...
    effects: {
         *setQuery({ payload }, { put, call, select }) {
            ...
            if (!error) {
                yield put({
                    type: 'setGroups',
                    payload: {
                        group: data._embedded && data._embedded.groups || [],
        ...
        *'view'({ payload:id }, {select, put, call}) {
            ...
            const err = yield parseError(error);
            yield message.error(`加载团购详情失败:${err.status} ${err.message}`, 3);
            return false;
        },
        *'add'({payload}, {select, put, call}) {
            ...
            if (!error) {
                yield message.success('创建团购信息成功', 2);
                yield put({
                    type: 'toggleSubmiting',
                    payload: false,
                });
                ...
                yield message.error(`创建团购信息失败:${err.status} ${err.message}`, 2);
                yield put({
                    type: 'toggleSubmiting',
                    payload: false,
                });
        ...
        *'update'({payload}, {select, put, call}) {
                    ...
                    if(!error) {
                        yield message.success(`更新团购信息成功`, 2);
                        yield put(routerRedux.goBack());
                        yield put({
                            type: "toggleSubmiting",
                            payload: false,
                        ...
                        const err = yield parseError(error);
                        yield message.error(`更新团购信息失败：${err.status} ${err.message}`, 2);
                        yield put({
                            type: "toggleSubmiting",
                            payload: false,
                        });
