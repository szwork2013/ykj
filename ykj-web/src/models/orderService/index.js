...
const getServiceTypeByPath = (location,state) => {
    const pathMapping = [{
        regxp : '/order/orderServices/:id/measures/*',
        type : 0,
        name : 'measures'
    },{
        regxp : '/order/orderServices/:id/designs/*',
        type : 1,
        name : 'designs'
    },{
        regxp : '/order/orderServices/:id/deliverys/*',
        type : 2,
        name : 'deliverys'
    },{
        regxp : '/order/orderServices/:id/installations/*',
        type : 3,
        name : 'installations'
    },{
        regxp : '/order/orderServices/:id/maintenances/*',
        type : 4,
        name : 'maintenances'
    }]
    ...
    pathMapping.map((item,index) =>{
        if(pathToRegexp(item.regxp).test(location.pathname)){
    ...
    subscriptions: {
        ...
        commonSubscriptions({ dispatch, history }) {
            ...
            history.listen((location, state) => {
                if (pathToRegexp('/order/orderServices/:id/*').test(location.pathname)) {
                    const match = pathToRegexp('/order/orderServices/:id/*').exec(location.pathname);
                    const orderId = match[1];
                    const serviceType = getServiceTypeByPath(location);
                    dispatch({
                        type: 'merge',
                        payload: {
         ...
         * 测量数据修改监听
         */
        measureEditSubscriptions({ dispatch, history }) {
            
            history.listen((location, state) => {
                if (pathToRegexp('/order/orderServices/:id/measures/edit/:measureId').test(location.pathname)) {
                    const match = pathToRegexp('/order/orderServices/:id/measures/edit/:measureId').exec(location.pathname);
                    const orderId = match[1];
                    const measureId = match[2];
                    console.log(orderId,measureId);
                    dispatch({
                        type: 'merge',
                        payload: {
                            currentItem : {
        ...
        setQuery: [function* ({ payload: query }, { put, select }) {
            ...
            if (!error) {
                yield put({
                    type: 'merge',
                    payload: {
                        list: data._embedded && data._embedded.measures || [],
                        pagination: {
