...
public class GoodController implements ResourceProcessor<RepositoryLinksResource> {
	...
	public ResponseEntity<?> getGoods(@PageableDefault Pageable pageable,
			@RequestParam(name = "isall", required = false) Boolean isAll, Authentication authentication) {
		return searchGoods(pageable, isAll, null, null, authentication);
	}

	@RequestMapping(value = "/search", method = RequestMethod.GET)
	public ResponseEntity<?> searchGoods(@PageableDefault Pageable pageable,
	...
	public ResponseEntity<?> getGood(@PathVariable("id") String id) {
	...
	public ResponseEntity<?> createGood(@RequestBody Good good, Authentication authentication) {
		...
		if (result) {
			return ResponseEntity
					.created(ControllerLinkBuilder
							.linkTo(ControllerLinkBuilder.methodOn(GoodController.class).getGood(good.getId())).toUri())
					.build();
		}

		return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
				.body(new GoodErrorBuilder(GoodErrorBuilder.ERROR_CREATED, "创建错误").build());
	}

	@RequestMapping(value = "/{id}", method = RequestMethod.PUT)
	public ResponseEntity<?> updateGood(@PathVariable("id") String id, @RequestBody Good good,
			Authentication authentication) {
		CustomUser customUser = (CustomUser) authentication.getPrincipal();
		String businessId = customUser.getClerk().getBusinessId();
		...
		if (result) {
			return ResponseEntity.noContent().location(ControllerLinkBuilder
					.linkTo(ControllerLinkBuilder.methodOn(GoodController.class).getGood(id)).toUri()).build();
		}

		return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
				.body(new GoodErrorBuilder(GoodErrorBuilder.ERROR_EDITED, "更新错误").build());
	}

	@RequestMapping(value = "/{id}", method = RequestMethod.DELETE)
	public ResponseEntity<?> deleteGood(@PathVariable("id") String id) {
		...
		return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
				.body(new GoodErrorBuilder(GoodErrorBuilder.ERROR_DELETED, "删除错误").build());
	}

	@RequestMapping(value = "/{id}/status_result", method = RequestMethod.PATCH)
	public ResponseEntity<?> statusResult(@PathVariable("id") String id,
			@RequestParam("onsaleStatus") Integer onsaleStatus, Authentication authentication) {
