...
public class MeasureController implements ResourceProcessor<RepositoryLinksResource> {
	...
	public ResponseEntity<?> getMeasure(@PathVariable("id") String id) {
	...
	public ResponseEntity<?> searchMeasures(@PageableDefault Pageable pageable,
		...
		if (isAll != null && isAll) {
			List<OrderSer> orders = measureService.selectOrderServiceWithDetailByCondition(customUser.getClerk(),
					orderServiceCondition);
			resources = listResourcesAssembler.toResource(orders, new MeasureResourceAssembler());
		} else {
			Page<OrderSer> orders = measureService.paginationOrderServiceWithDetailByCondition(customUser.getClerk(),
					orderServiceCondition, pageable);
			resources = pagedResourcesAssembler.toResource(orders, new MeasureResourceAssembler());
		}
		...
		if (result) {
			return ResponseEntity.noContent()
					.location(ControllerLinkBuilder
							.linkTo(ControllerLinkBuilder.methodOn(MeasureController.class).getMeasure(id)).toUri())
					.build();
		}
	...
	public ResponseEntity<?> deleteMeasure(@PathVariable("id") String id, Authentication authentication) {
		CustomUser customUser = (CustomUser) authentication.getPrincipal();
		...
		if (measureService.deleteOrderService(customUser.getClerk(), id)) {
