...
public class DesignController implements ResourceProcessor<RepositoryLinksResource> {
	...
	public ResponseEntity<?> getDesign(@PathVariable("id") String id) {
		if (StringUtils.isBlank(id)) {
			return ResponseEntity.status(HttpStatus.NOT_FOUND)
					.body(new OrderServiceErrorBuilder(OrderServiceErrorBuilder.ERROR_ID_NULL, "设计服务编号为空").build());
		}
		OrderSer design = designService.findById(id);
		if (design == null) {
			return ResponseEntity.status(HttpStatus.NOT_FOUND)
					.body(new OrderServiceErrorBuilder(OrderServiceErrorBuilder.ERROR_SERVICE_NULL, "找不到设计服务").build());
		}
	...
	public ResponseEntity<?> createDesign(@RequestBody OrderSer design, Authentication authentication) {
		CustomUser customUser = (CustomUser) authentication.getPrincipal();
		...
		String id = designService.createOrderService(clerk, design);
	...
	public ResponseEntity<?> updateDesign(@PathVariable("id") String id, @RequestBody OrderSer design,
			Authentication authentication) {
		CustomUser customUser = (CustomUser) authentication.getPrincipal();
		Clerk clerk = customUser.getClerk();
		Date date = new Date();
		...
		Boolean result = designService.updateOrderService(clerk, design);

		if (result) {
			return ResponseEntity.noContent()
					.location(ControllerLinkBuilder
							.linkTo(ControllerLinkBuilder.methodOn(DesignController.class).getDesign(id)).toUri())
					.build();
		}
	...
	public ResponseEntity<?> statementDesign(@PathVariable("id") String id) {
		if (StringUtils.isBlank(id)) {
			return ResponseEntity.status(HttpStatus.NOT_FOUND)
					.body(new OrderServiceErrorBuilder(OrderServiceErrorBuilder.ERROR_ID_NULL, "设计服务编号为空").build());
		}
		...
		if (design == null) {
			return ResponseEntity.status(HttpStatus.NOT_FOUND)
					.body(new OrderServiceErrorBuilder(OrderServiceErrorBuilder.ERROR_SERVICE_NULL, "找不到测量服务").build());
		}
		...
		Boolean result = designService.statement(design);
	...
	public ResponseEntity<?> cancelStatementDesign(@PathVariable("id") String id) {
		if (StringUtils.isBlank(id)) {
			return ResponseEntity.status(HttpStatus.NOT_FOUND)
					.body(new OrderServiceErrorBuilder(OrderServiceErrorBuilder.ERROR_ID_NULL, "设计服务编号为空").build());
		}
		...
		Boolean result = designService.cancelStatement(design);
	...
	public ResponseEntity<?> deleteMeasure(@PathVariable("id") String id) {
